...
export interface AppConfig {
  ...
  // daemon mode and hot‑word settings
  daemon?: boolean;
  wakeWord?: string;
  audio: { // optional voice activation settings
    inputDevice?: string;
    outputDevice?: string;
  ...
  for (let i = 2; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      case '--no-stt':
        ...
        case '--wake-word':
            if (next && !next.startsWith('-')) {
                result.wakeWord = next;
                i++;
            }
            break;
        case '--voice-activate':
            result.voiceActivate = true;
            break;
    case '--voice-key':
      if (next && !next.startsWith('-')) {
        result.voiceKey = next;
        i++;
      }
      break;
    case '--auto-detect':
      result.audio = result.audio || {};
      result.audio.autoDetect = true;
      break;
    case '--hot-word':
      if (next && !next.startsWith('-')) {
        result.audio = result.audio || {};
        result.audio.hotWords = (result.audio.hotWords || []).concat(next);
        i++;
      }
      break;
      ...
      default:
        ...
        if (arg && !result.command && !arg.startsWith('-')) {
          result.command = arg;
        }
        break;
    }
...
Options:
  ...
  --no-tts            Disable TTS
  --no-stt            Disable STT
  --skip-opencode     Skip OpenCode server check (for testing)
  --no-stt            Disable STT
...
export function loadConfig(args?: string[]): AppConfig {
  ...
  return {
    ...
    audio: {
      inputDevice: cliArgs.audio?.autoDetect ? undefined : process.env.AUDIO_INPUT_DEVICE,
      outputDevice: cliArgs.audio?.autoDetect ? undefined : process.env.AUDIO_OUTPUT_DEVICE,
      voiceActivation: cliArgs.voiceActivate ?? false,
      activationKey: cliArgs.voiceKey || 'ENTER',
      autoDetect: cliArgs.audio?.autoDetect ?? false,
      hotWords: cliArgs.audio?.hotWords ?? [],
    },

    daemon: cliArgs.daemon ?? false,
    wakeWord: cliArgs.wakeWord || 'opencode',
  };
